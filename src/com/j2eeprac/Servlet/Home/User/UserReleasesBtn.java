...
@WebServlet("/UserReleasesBtn")
public class UserReleasesBtn extends HttpServlet {
	@Override
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (userProfileName == null || userProfileName.equals("Î´µÇÂ¼")) {
			...
			List<Article> articleList = articleDao.selectAllByAuthor(userProfileName);
			session.setAttribute("userProfile", null);
			session.setAttribute("userReleases", articleList);
			session.setAttribute("userList", null);
			session.setAttribute("articleList", null);
